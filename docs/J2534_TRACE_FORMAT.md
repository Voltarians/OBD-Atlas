...
The native Windows proxy now uses the same trace semantics while forwarding only the currently accepted device, channel, filter, and read-only message APIs to a selected provider. Atlas must never generate an extra request merely because tracing is enabled. `PassThruWriteMsgs` is not exported in the current stage.
...
- `callId`
- `api`, for example `PassThruOpen`, `PassThruConnect`, `PassThruStartMsgFilter`, `PassThruReadMsgs`, or later `PassThruWriteMsgs`
- `threadId`
...
Read-message tracing currently retains the exact provider-returned payload bytes during the off-vehicle acceptance stage so direct-vs-proxy equivalence can be proven. The proxy does not guess a diagnostic service from arbitrary transport bytes. Vehicle use remains blocked until the later message, IOCTL, programming-voltage, privacy/redaction, and bench-safety gates are completed.

## Sensitive payload policy

By default in the platform-independent trace library:
...
Explicit inclusion is supported by the Python library for controlled bench/research cases, but it is not the default Atlas policy. The native proxy's read-only stage is still off-vehicle and is not approved for sensitive production/programming sessions.
...
Windows CI compares direct fake-provider behavior with proxied behavior for device lifecycle, channel lifecycle, ISO15765 flow-control filter setup/teardown, successful multi-message reads, and timeout reads. It also verifies fail-closed provider identity and stress-tests concurrent trace ordering. No `PassThruWriteMsgs`, IOCTL, or programming-voltage API is enabled yet.
...
Device Open/Close, Connect/Disconnect, Start/StopMsgFilter, and ReadMsgs are now staged behind direct-vs-proxy tests, but the overall vehicle/programming gate remains closed until the remaining write/IOCTL/programming-voltage and bench-safety stages pass independently.
