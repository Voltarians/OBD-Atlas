...
OBD Atlas 0.1.0 Alpha 1 is an early research release. Download the file matching the host from the GitHub Releases page.
...
```bash
sudo apt update
sudo apt install -y libgtk-3-0 can-utils
```

Extract `OBD-Atlas-Linux-arm64-v0.1.0-alpha.1.zip`, then run:
