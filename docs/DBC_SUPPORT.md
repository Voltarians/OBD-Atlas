...
The Flutter application exposes this capability through the **DBC** navigation
destination. Imported definitions remain in local application storage, retain
a SHA-256 provenance record, and can be exported through the system file picker
without an internet connection. The Python CLI remains the authoritative
normalized SQLite import/export path described below.
